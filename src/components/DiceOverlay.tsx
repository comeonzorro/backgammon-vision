...
export function DiceOverlay({ frame, show, status }: Props) {
  ...
  const searching = status === "searching" || status === "tracking" || status === "rolling";

  return (
    <div className={styles.overlay}>
      {searching && frame?.dice.length === 0 && (
        <span className={styles.hint}>Recherche des dés sur le plateau…</span>
      )}
